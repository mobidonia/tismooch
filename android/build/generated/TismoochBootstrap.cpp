...
#define TAG "com.mobidonia.smooch"

using namespace v8;

static Persistent<Object> bindingCache;

static Handle<Value> Tismooch_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("Tismooch.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::TismoochBindings::lookupGeneratedInit(
...
static void Tismooch_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(Tismooch_getBinding)->GetFunction());
}

static void Tismooch_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::TismoochBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry TismoochBinding = {
	"com.mobidonia.smooch",
	Tismooch_init,
	Tismooch_dispose
};
...
Java_com_mobidonia_smooch_TismoochBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("com.mobidonia.smooch", &TismoochBinding);
	titanium::KrollBindings::addExternalLookup(&(::TismoochBindings::lookupGeneratedInit));
}
